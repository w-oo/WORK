...
def eval_alignment_by_sim_mat(embed1, embed2, top_k, nums_threads, m, csls=0, accurate=False,output = True):
    t = time.time()
    sim_mat = sim_handler(embed1, embed2, csls, nums_threads, m)
...
def sim_handler(embed1, embed2, k, nums_threads, m):
    sim_mat = np.matmul(embed1, embed2.T)
    alpha = 0.3
    sim_mat_t = m  
    sim_mat = (1-alpha)*sim_mat + alpha*(sim_mat_t) 
